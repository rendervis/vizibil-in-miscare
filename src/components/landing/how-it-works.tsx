    ...
    <section id="cum-functioneaza" className="paper-grid bg-[var(--paper)] py-20 md:py-28">
      <div className="mx-auto max-w-7xl px-5 md:px-8">
        <div className="mx-auto max-w-3xl text-center">
          <Badge className="border-[var(--ink)]/10 bg-white/65 text-[var(--ink)]/68">Cum funcționează</Badge>
          <h2 className="font-display mt-5 text-5xl font-extrabold leading-[.94] tracking-[-.055em] md:text-7xl">Trei pași. Fără jargon.</h2>
          <p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-[var(--ink)]/70">Publicitatea se mută dintr-un panou fix pe mașini care circulă deja prin oraș.</p>
        </div>
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-3">
          {steps.map(({ icon: Icon, n, title, body, image, alt }) => (
            <article key={n} className="overflow-hidden rounded-[2rem] border border-[var(--ink)]/10 bg-white/70 shadow-[0_18px_70px_rgba(31,24,48,.06)]">
              <MediaFrame src={image} alt={alt} className="aspect-[4/3]" />
