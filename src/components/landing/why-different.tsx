      ...
      <div className="mx-auto max-w-7xl px-5 md:px-8">
        <div className="grid gap-8 lg:grid-cols-[.75fr_1.25fr] lg:items-end">
          <div>
            ...
            <h2 className="font-display mt-5 text-5xl font-extrabold leading-[.94] tracking-[-.055em] md:text-6xl">Nu e doar un magnet pus pe o mașină.</h2>
          </div>
          <p className="max-w-2xl text-base leading-7 text-[var(--ink)]/58 lg:justify-self-end">Valoarea vine din potrivirea dintre zonă, traseu și campanie — apoi din faptul că livrarea poate fi verificată.</p>
        </div>
        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {points.map(({ icon: Icon, title, body }) => (
            <article key={title} className="rounded-[2rem] border border-[var(--ink)]/10 bg-white/72 p-7">
              ...
              <p className="mt-3 text-sm leading-6 text-[var(--ink)]/48">{body}</p>
            </article>
          ))}
        </div>
      </div>
